...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Methuselah database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Methuselah datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Methuselah datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Methuselah 0.7 datadir."
    ;;
  3)
    echo "Detected Methuselah pre-0.8 datadir."
    ;;
  4)
    echo "Detected Methuselah 0.8 datadir."
    ;;
esac
