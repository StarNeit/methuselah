...
void CMasternodeMan::ProcessMessage(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, CConnman& connman)
{
    ...
    if (strCommand == "dsee") { //DarkSend Election Entry
        ...
        if(addr.GetPort() != 7555 && Params().NetworkIDString() == CBaseChainParams::MAIN) {
            LogPrintf("dsee - Got bad mainnet Masternode port: Actual:%d => Expected:%d\n", addr.GetPort(), 7555);
            return;
        
        }
        else if(addr.GetPort() != 7444 && Params().NetworkIDString() == CBaseChainParams::TESTNET) {
        ...
        if(AcceptableInputs(mempool, state, MakeTransactionRef(mtx))){
            ...
            {
                CBlockIndex* pMNIndex = (*mi).second; // block for 9600 SAP tx -> 1 confirmation
