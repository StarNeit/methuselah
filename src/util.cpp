...
/**TODO-- port to methuselah*/
//Methuselah only features
...
const char * const METHUSELAH_CONF_FILENAME = "methuselah.conf";
const char * const METHUSELAH_PID_FILENAME = "methuselah.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            if (mapMultiArgs.count("-debug")) {
				...
				/**TODO-- */
				// "methuselah" is a composite category enabling all Methuselah-related debug output
				if(ptrCategory->count(string("methuselah"))) {
...
#else
    const char* pszModule = "methuselah";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\methuselah
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\methuselah
    // Mac: ~/Library/Application Support/methuselah
    // Unix: ~/.methuselah
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "methuselah";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/methuselah";
#else
    // Unix
    return pathRet / ".methuselah";
#endif
#endif
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
		// [methuse] FIX: Create empty methuselah.conf if it does not exist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override methuselah.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", METHUSELAH_PID_FILENAME));
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Check for untranslated substitution to make sure Methuselah Core copyright is not removed by accident
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Methuselah Core") == std::string::npos) {
