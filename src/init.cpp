 ...
 * between methuselahd, and methuselah-qt and non-server tools.
*/
...
void PrepareShutdown() //TODO--
{
    ...
    RenameThread("methuselah-shutoff");
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), METHUSELAH_CONF_FILENAME));
    if (mode == HMM_METHUSELAHD)
    {
#if HAVE_DECL_DAEMON
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), METHUSELAH_PID_FILENAME));
#endif
    ...
    if (showDebug)
    {
        ...
        strUsage += HelpMessageOpt("-sporkkey=<privkey>", strprintf("Enable spork administration functionality with the appropriate private key."));
    }
	/**TODO-- */
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/methuselah-coin/methuselah>";
...
void ThreadImport(std::vector<boost::filesystem::path> vImportFiles)
{
    ...
    RenameThread("methuselah-loadblk");
...
/** Sanity checks
 *  Ensure that Methuselah is running in a usable environment with all
...
void InitLogging()
{
    ...
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("methuselah version %s\n", FormatFullVersion());
}
...
static bool LockDataDirectory(bool probeOnly)
{
    ...
    // Make sure only a single methuselah process is using the data directory.
    ...
    try {
       ...
       // Wait maximum 10 seconds if an old wallet is still running. Avoids lockup during restart
        if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
            return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Methuselah is probably already running."), strDataDir));
        if (probeOnly) {
            lock.unlock();
        }
...
bool AppInitMain(boost::thread_group& threadGroup, CScheduler& scheduler)
{
    ...
    LogPrintf("Using config file %s\n", GetConfigFile(GetArg("-conf", METHUSELAH_CONF_FILENAME)).string());
	...
	// [methuse] FIX: added spork key param.
	if (GetArg("-sporkkey", "") != "") // spork priv key
    {
	...
	if (mapMultiArgs.count("-masternodepaymentskey")) // masternode payments priv key
    {
        if (!masternodePayments.SetPrivKey(GetArg("-masternodepaymentskey", ""))){
			...
			//LogPrintf("Masternode: Create new key %s", CMethuselahSecret(secret).ToString());
   ...
   //if (!fDisableWallet) {
    ...
    if (GetBoolArg("-minetolocalwallet", false)) {
        return InitError(_("Methuselah was not built with wallet support. Set -minetolocalwallet=0 to use -mineraddress, or rebuild Methuselah with wallet support."));
    }
    if (GetArg("-mineraddress", "").empty()) {
        return InitError(_("Methuselah was not built with wallet support. Set -mineraddress, or rebuild Methuselah with wallet support."));
    }
 #endif // !ENABLE_WALLET
    ...
    if (mapMultiArgs.count("-mineraddress")) {
        ...
        if (pwalletMain) {
            ...
            CMethuselahAddress addr(GetArg("-mineraddress",""));
            CKeyID keyID;
            addr.GetKeyID(keyID);
