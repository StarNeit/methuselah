...
/*************************************** 
 * Bitcoin Cash D601 Algorithm Adapted *
 * dev@methuselahcoin.io                     *
 ***************************************/
 ...
 */
uint32_t GetNextWorkRequiredD601(const CBlockIndex *pindexPrev,
    ...
    // If the new block's timestamp is more than 2*n seconds then allow
...
unsigned int GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock, const Consensus::Params& params)
{
    ...
    if (pindexLast->nHeight <= params.fPowD601SwitchHeight)
        return GetNextWorkRequiredZMDS(pindexLast, pblock, params);

    return GetNextWorkRequiredD601(pindexLast, pblock, params);
}
